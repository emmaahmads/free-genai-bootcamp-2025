from flask import request, jsonify, g, Blueprint, jsonify, request
...
def load(app):
  ...
  @groups_bp.route('/groups/<int:id>/words/raw', methods=['GET'])
def get_group_words_raw(id):
    try:
        cursor = g.db.cursor()
        cursor.execute('SELECT * FROM words WHERE group_id = ?', (id,))
        words = cursor.fetchall()
